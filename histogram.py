...
class histogram :
    ...
    def show(self, which_hist= None , axis=None ,show_fit=False, slice = None, config = None,scale='linear', color = 'k', setylim = True ):
        ...
        ax=axis
        if type(axis).__name__=='NoneType':
            plt.figure()
            ax=plt
        ...
        ax.step(self.bin_centers[slice[0]:slice[1]:slice[2]], self.data[which_hist][slice[0]:slice[1]:slice[2]])
        print(self.bin_centers[slice[0]:slice[1]:slice[2]])
        print(self.data[which_hist][slice[0]:slice[1]:slice[2]])
